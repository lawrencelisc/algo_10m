...
class DataCenterSrv:
    ...
    def create_df(self):
        ...
        for _, row in self.strat_df.iterrows():
            ...
            filename_a: str = f'{name}_{symbol}_a.csv'
            file_path = self.data_folder_GN / filename
            file_path_a = self.data_folder_GN / filename_a
            ...
            df10 = self.get_bybite_data(unix_bybit_get_since, symbol)

            if combined_df.empty:
                gn_bybit_df = pd.concat(
                    [existing_df.dropna(how='all', axis=1),
                     df10.dropna(how='all', axis=1)],
                    axis=0
                )
            else:
                gn_bybit_df = pd.concat(
            ...
            try:
                gn_bybit_df.to_csv(file_path_a)
                logger.info(
                    f'Overwrite empty file with {len(df_new)} gn rows + '
                    f'{len(df10)} bybit get data for {symbol}: {filename_a}'
                            )
            except Exception as e:
                logger.error(f'Failed to save CSV for {symbol}: {e}')
