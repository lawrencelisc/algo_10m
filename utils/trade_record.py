    ...
    def hr_traded(self):
        ...
        if os.path.exists(self.signal_path):
            try:
                ...
                df = pd.DataFrame()
                logger.error(f'Failed to read existing CSV {self.signal_path}: {e}')
                return False
            except Exception as e:
                logger.error(f'Error reading {self.signal_path}: {e}')
                return False
        else:
            df = pd.DataFrame()
        ...
        last_ts: str = df['date'].iloc[-1]
        unix_last_ts: float = float((pd.to_datetime(last_ts)).timestamp())
        unix_diff_ts: float = (unix_now_ts - unix_last_ts)
        return (unix_diff_ts < 2 * 60 * 60)
